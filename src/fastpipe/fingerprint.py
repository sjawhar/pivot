...
Validated: See /workspaces/treeverse/fastpipe/experiments/test_getclosurevars_approach.py
...
from fastpipe import ast_utils
...
def get_stage_fingerprint(func: Any, visited: set[int] | None = None) -> dict[str, str]:
    ...
    for name, value in closure_vars.globals.items():
        ...
        if callable(value) and is_user_code(value):
            manifest[f"func:{name}"] = hash_function_ast(value)
            child_manifest = get_stage_fingerprint(value, visited)
            ...
            for mod_name, attr_name in attrs:
                if mod_name == name or (hasattr(value, "__name__") and value.__name__ == mod_name):
...
def is_user_code(obj: Any) -> bool:
    ...
    for stdlib_path in stdlib_paths:
        try:
            if stdlib_path in module_file.parents and (
                "site-packages" not in module_file.parts
                and "dist-packages" not in module_file.parts
            ):
                return False
        except (ValueError, OSError):
            continue

    if "site-packages" in module_file.parts or "dist-packages" in module_file.parts:
        return False

    try:
        cwd = Path.cwd()
        module_file.relative_to(cwd)
        return True
    except ValueError:
        pass

    # Could be in a different project directory
    return True
