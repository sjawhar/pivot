# Fastpipe Source Code Documentation

**Directory:** `/workspaces/treeverse/fastpipe/src/fastpipe/`
**Updated:** 2026-01-04
...
This directory contains all Fastpipe source code organized by functionality.
...
- `stage` - Decorator for marking functions as pipeline stages
...
```python
{
    ...
    'fingerprint': dict[str, str],  # Code dependency manifest
}
```
...
**Commands:**
- `fastpipe run [stages] [--workers N] [--force] [--explain] [--executor TYPE]`
- `fastpipe status` - Show which stages need re-run
- `fastpipe dag [--save FILE]` - Visualize pipeline
- `fastpipe export [--output FILE] [--validate]` - Generate dvc.yaml
...
from fastpipe.fingerprint import get_stage_fingerprint
...
def analyze_function(func):
    ...
    fp = get_stage_fingerprint(func)  # From fastpipe?
...
if TYPE_CHECKING:
    ...
    from fastpipe.types import StageInfo
```
...
```python
# Good
def get_stage_fingerprint(func: Callable, visited: set[int] | None = None) -> dict[str, str]:
    ...
