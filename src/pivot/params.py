    ...
    for stage in sorted(all_stages):
        ...
        for key in sorted(all_keys):
            ...
            if key not in old_params:
                diffs.append(ParamDiff(stage=stage, key=key, old=None, new=new_val, change="added"))
            elif key not in new_params:
                diffs.append(
                    ParamDiff(stage=stage, key=key, old=old_val, new=None, change="removed")
                )
            elif not _values_equal(old_val, new_val):
                diffs.append(
                    ParamDiff(stage=stage, key=key, old=old_val, new=new_val, change="modified")
                )
