                ...
                for future in done:
                    ...
                    try:
                        ...
                        if result["status"] == StageStatus.FAILED:
                            _handle_stage_failure(stage_name, stage_states, error_mode, lifecycle)
                        ...
                        _mark_stage_failed(
                            state, f"Worker died: {e}", stage_states, error_mode, lifecycle
                        )
                        logger.error(f"Worker process died while running '{stage_name}'")

                    except Exception as e:
                        _mark_stage_failed(state, str(e), stage_states, error_mode, lifecycle)
...
def _start_ready_stages(
    ...
    for stage_name, state in stage_states.items():
        ...
        try:
            ...
            for mutex in state.mutex:
                mutex_counts[mutex] -= 1
            _mark_stage_failed(state, f"Failed to submit: {e}", stage_states, error_mode, lifecycle)
...
def _mark_stage_failed(
    ...
    if lifecycle:
        ...
        state.status = StageStatus.FAILED
        state.end_time = time.perf_counter()
    _handle_stage_failure(state.name, stage_states, error_mode, lifecycle)
