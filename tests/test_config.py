...
from pivot import runtime_config

# --- load_config tests ---


def test_load_config_missing_file_returns_empty(tmp_path: pathlib.Path) -> None:
    result = runtime_config.load_config(tmp_path / "nonexistent.yaml")
    assert result == runtime_config.PivotConfig()
...
def test_load_config_parses_checkout_mode(tmp_path: pathlib.Path) -> None:
    ...
    result = runtime_config.load_config(config_file)
...
def test_load_config_cached_per_process(tmp_path: pathlib.Path) -> None:
    ...
    first = runtime_config.load_config(config_file)

    # Modify file - should not affect cached result
    config_file.write_text("cache:\n  checkout_mode:\n    - copy\n")

    second = runtime_config.load_config(config_file)
...
def test_load_config_uses_project_root(
    ...
    result = runtime_config.load_config()
...
def test_get_checkout_mode_order_default() -> None:
    empty_config = runtime_config.PivotConfig()

    result = runtime_config.get_checkout_mode_order(empty_config)

    assert result == ["hardlink", "symlink", "copy"]


def test_get_checkout_mode_order_from_config() -> None:
    test_config = runtime_config.PivotConfig(
        cache=runtime_config.CacheConfig(checkout_mode=["symlink", "hardlink"])
    )

    result = runtime_config.get_checkout_mode_order(test_config)
...
def test_get_checkout_mode_order_invalid_mode_skipped() -> None:
    test_config = runtime_config.PivotConfig(
        cache=runtime_config.CacheConfig(checkout_mode=["hardlink", "invalid_mode", "copy"])
    )
...
def test_get_checkout_mode_order_all_invalid_returns_default() -> None:
    test_config = runtime_config.PivotConfig(
        cache=runtime_config.CacheConfig(checkout_mode=["invalid1", "invalid2"])
    )
...
def test_get_checkout_mode_order_empty_list_returns_default() -> None:
    test_config = runtime_config.PivotConfig(cache=runtime_config.CacheConfig(checkout_mode=[]))
...
def test_get_checkout_mode_order_loads_config_when_none(
    ...
    result = runtime_config.get_checkout_mode_order()
...
def test_get_checkout_mode_order_returns_copy() -> None:
    test_config = runtime_config.PivotConfig(
        cache=runtime_config.CacheConfig(checkout_mode=["hardlink"])
    )

    result1 = runtime_config.get_checkout_mode_order(test_config)
    result2 = runtime_config.get_checkout_mode_order(test_config)
