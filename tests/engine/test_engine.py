...
@pytest.mark.parametrize(
    ("param_name", "param_value"),
    [
        pytest.param("explain_mode", True, id="explain_mode"),
        pytest.param("show_output", True, id="show_output"),
    ],
)
def test_engine_run_once_accepts_compatibility_parameter(
    param_name: str, param_value: object
) -> None:
    """run_once() accepts compatibility parameters without passing to orchestration."""
    with patch.object(engine.Engine, "_orchestrate_execution", return_value={}) as mock_orchestrate:
        eng = engine.Engine()
        eng.run_once(**{param_name: param_value})  # pyright: ignore[reportArgumentType]

        # These parameters are accepted but not passed to _orchestrate_execution
        call_kwargs = mock_orchestrate.call_args.kwargs
